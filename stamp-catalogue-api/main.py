...
def startup_event():
    """Initialize the API by initializing the database. That means opening the
       local CSV-file containing a full stamp catalogue and then reading into
       some suitable in-memory data structure."""
    global db
    if os.path.exists(STAMP_CATALOGUE_CSV_FILE):
        with open(STAMP_CATALOGUE_CSV_FILE) as f:
            db = pd.read_csv(STAMP_CATALOGUE_CSV_FILE, delimiter=';') #, index_col="yt")
            print("1a" in db.index)
...
def api_root_resource (request: Request):
    """The root resource. Returns the name of this API and an URL where the OpenAPI specification of this API can be found."""
    print(request.url)
...
def get_stamps (response: Response,
                title: Optional[str] = Query(None, description="Return stamps that have the given `title`. "),
                year: Optional[str] = Query(None, description="Return stamps that were issued the given `year`. `year` can be a comma-separated list of years."),
                color: Optional[str] = Query(None, description="Return stamps with the given `color`. `color` can be a comma-separated list of colors."),
                value: Optional[str] = Query(None, description="Return stamps with the given printed `value`. `value` can be a comma-separated list of values."),
                stamp_type: Optional[str] = Query(None, description="Return stamps of the given `stamp_type`. `stamp_type` can be a comma-separated list of values."),
                start: Optional[int] = Query(None, description="Return the stamps beginning with stamp at `start` position in the list of stamps. If not given, `start` is implicitly 1. Must be >= 1."),
                count: Optional[int] = Query(None, description="Return the given `count` of stamps from the list of stamps. If not given, `count` is implicitly 'all'. Must be >= 1.")):
    """Return the catalogue of stamps. If no query parameter is specified all stamps in the catalogue are returned. All query parameters can be combined.

The query parameters `title`, `year`, `color`, `value` and `stamp_type` can be used to filter which stamps to return from the catalogue.

The query parameters `start`and `count` control which and how many number of the, possibly filtered, stamps in the catalogue to return.
...
* `/stamps?value=1 French centime` will return all stamps with the printed value of "1 French centime".
...
* `/stamps?count=100` will return a maximum of 100 stamps.
    ...
    # I should add a column with the URL to each individual stamp and to the URL for the image of each individual stamp
...
def get_stamp (response: Response, yt: str = Path (None, description="`yt` is the Yvert-Tellier catalogue number that identifies a specific stamp.")):
...
def get_stamp_image (response: Response, yt: str = Path (None, description="`yt` is the Yvert-Tellier catalogue number that identifies a specific stamp.")):
